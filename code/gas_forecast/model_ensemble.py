...
class GasAwareEnsembleForecaster(RidgeDeltaForecaster):
    ...
    def _predict_branches(
        ...
        residual = np.column_stack(
            [model.predict(x) for model in models.residual_models]
        )
        lgb_residual = ridge + residual
        deltas = [
            np.zeros_like(ridge),
            ridge,
            recent,
    ...
    def _event_matrix(
        ...
        probabilities = gmm.predict_proba(anchor[:, None])
        distances = np.abs(anchor[:, None] - gmm.means_.reshape(1, -1))
        matrix = x.reindex(columns=columns).reset_index(drop=True).copy()
        for state in range(probabilities.shape[1]):
            matrix[f"state_probability_{state}"] = probabilities[:, state]
            matrix[f"state_distance_{state}"] = distances[:, state]
        return matrix
    ...
    def _fit_event_state(
        ...
        components = min(self.config.model.state_components, max(2, len(x) // 200))
        gmm = GaussianMixture(
    ...
    def _gate_matrix(
        ...
        matrix["model_disagreement"] = np.std(branches[:, 1:, step], axis=1)
    ...
    def _fit_gates(
        ...
        for step in range(actual.shape[1]):
            corrected = branches[:, 1:, step] @ correction_weights[:, step]
            label = (
                np.abs(actual[:, step] - corrected) + self.config.model.gate_margin
                < np.abs(actual[:, step] - anchor)
            ).astype(int)
            if len(np.unique(label)) < 2:
                gates.append(GateState(model=None, constant_probability=float(label[0])))
                continue
        ...
        for target in self.config.targets:
            ...
            if self.version == "v3":
                ...
                event_state = self._fit_event_state(
                    x, anchor.to_numpy(dtype=float), y.to_numpy(), target, gate_columns
                )
        ...
        for target in self.config.targets:
            ...
            if self.version == "v2":
                ...
                if state.event is None:
                    raise RuntimeError("V3 缺少状态迁移模型")
                events = self._predict_events(state.event, x, anchor)
                ...
                for step, gate in enumerate(state.gates):
                    ...
                    if gate.model is None:
                        ...
                        probability = gate.model.predict_proba(gate_x)[:, 1]
                    disagreement = np.std(branches[:, 1:, step], axis=1)
                    scale = self.config.model.uncertainty_scale * np.maximum(np.abs(anchor), 1.0)
                    conservative = 1.0 / (1.0 + disagreement / scale)
                    absolute[:, step] = anchor + probability * conservative * (corrected - anchor)
